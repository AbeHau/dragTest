package com.example.shimo.dragtest;
...
public class MainActivity extends AppCompatActivity {
        ...
        public boolean onDrag(View v, DragEvent event) {
            ...
            switch (event.getAction()) {
                case DragEvent.ACTION_DRAG_LOCATION:
                    ...
                    if (event.getY() -  scrollY > mScrollView.getBottom() - 50) {
                        startScrolling(scrollY, mGrid.getHeight());
                    } else if (event.getY() - scrollY < mScrollView.getTop() + 50) {
    ...
    private void startScrolling(int from, int to) {
        if (from != to && mAnimator == null) {
            ...
            mAnimator.setDuration(Math.abs(to - from)*4);
